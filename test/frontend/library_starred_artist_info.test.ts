...
for (const [name, locale] of [["zh-CN", zh], ["en", en]] as const) {
  for (const key of ["tabStarred", "noStarred", "starredLoadFailed", "sortLabel", "sortNewest", "sortOldestStarred", "sortNewestAdded", "sortOldestAdded", "sortNameAsc", "sortNameDesc", "like", "unlike", "starUpdateFailed", "artistInfoTitle", "artistInfoLoading", "artistInfoUnavailable", "artistInfoOpen"]) {
    assert(typeof locale.library?.[key] === "string", `${name} library.${key} exists`);
  }
}
assert(zh.app?.menu?.starred === "我喜欢", "Chinese sidebar uses 我喜欢 for the starred route");
